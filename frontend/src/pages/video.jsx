...
export default function VideoDetection() {
  ...
  const recordVideo = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:5000/record_video");
      const result = response.data;
      console.log(result);
      setDetectionStatus(`Detection complete! ${result}`);
      await fetchVideos();
      setDetectionStatus("Ready to capture");
    } catch (error) {
      console.error("Error recording video:", error);
  ...
  return (
    <div className="app-container">
      <div className="content-wrapper">
        <header className="header">
          <h1>Smart Door Lock with Face Recognition</h1>
          <p>Identify known visitors and record video</p>
        </header>
        ...
        <section className="controls">
          ...
          >
            {loading ? "Processing..." : "Capture Video"}
          </button>
          <p className="status">{detectionStatus}</p>
        </section>
        ...
        <footer className="footer">
          <p>Smart Door Lock System • {new Date().getFullYear()}</p>
        </footer>
      </div>
    </div>
  );
